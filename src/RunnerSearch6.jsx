...
function RunnerSearch() {
  ...
  const handleSearch = async () => {
    if (!query) return;
    setLoading(true);
    setResult(null); // ล้างผลลัพธ์ก่อน
    try {
    ...
    <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow-lg mt-10 relative">
      ...
      <div className="flex space-x-2 mb-6">
        ...
        <button
          ...
          className={`px-4 rounded-lg transition text-white ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
          disabled={loading}
        >
          {loading ? text[lang].searching : text[lang].search}
        </button>
      </div>

      {/* ผลลัพธ์ */}
      {loading && (
        <p className="text-center text-gray-600 font-medium mb-6">{text[lang].searching}</p>
      )}
